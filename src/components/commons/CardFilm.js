  ...
  const handleAdd = async (item) => {
    if (user) {
      try {
        ...
        let checkFilm = docSnap
          .data()
          .bookmark.some((item2) => item2.id === item.id);
        if (checkFilm) {
      ...
      <div className="bg-black-color p-4 rounded-md">
        <div className="mb-6 relative">
          ...
          <div className="card-details absolute  text-center left-1/2 top-1/2 -translate-y-1/2 w-32 -translate-x-1/2 opacity-0 ">
            <p
              className="btn px-0 mb-3 bg-yellow-color border-none text-black-color -translate-y-1/2 transition-all duration-1000 card-btn opacity-0 "
              onClick={() => {
                handleAdd(item);
              }}
            >
              {listFilm.find((film) => film.id === item.id)
                ? "Film is available "
                : "Add bookmark"}
            </p>
            <Link to={`/${item.id}`}>
