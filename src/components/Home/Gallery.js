...
export default function Gallery() {
    ...
    useEffect(()=>{
        axios.get(request.requestUpcomming)
        .then(function(res){
    ...
    <div  className='bg-gallery_bg w-full py-32 relative'>
        ...
        <Slider ref={sliderRef}  {...settings} >
            { upcoming.map((item,index)=>{
                return (
                    <div key = {item.id} className={` px-7 opacity-100 transition-all duration-1000  ${index === slideindex ? 'slide-active' : 'opacity-10  mt-20' }`}>
                        <img className='' src={`https://image.tmdb.org/t/p/original${item.backdrop_path}`}></img>
                    </div>
                )
            })}
        </Slider>
        ...
        <div className="flex mx-60  justify-between text-white">
            ...
            <p className='font-bold hover:text-yellow-color hover:cursor-pointer ' onClick={()=>sliderRef.current.slickNext()}>
                NEXT
